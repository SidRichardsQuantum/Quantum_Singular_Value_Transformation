...
5. Confirm the wheel smoke step installs the built wheel in a fresh virtual
   ...
   schema CLI commands.
6. Confirm branch coverage remains above the configured project floor and the
   standalone README quick examples execute under the test suite.
7. Confirm generated outputs remain untracked except for deliberate research
   artifacts under `results/`.
8. Publish only after the tagged commit's Ordered Actions run passes lint,
