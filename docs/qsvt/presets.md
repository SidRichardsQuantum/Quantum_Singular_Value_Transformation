...
The `qsvt.presets` module provides ready-to-use bounded polynomial families for common Quantum Singular Value Transformation (QSVT) and Quantum Signal Processing (QSP) style experiments. It is intended as a practical starting point when you want a useful polynomial quickly, without setting up a full approximation workflow by hand.
...
In many QSVT workflows, the goal is not to derive a best-possible polynomial from scratch, but to obtain a small bounded polynomial with the right qualitative behaviour. The `qsvt.presets` module provides exactly that: a collection of simple polynomial families that are easy to inspect, evaluate, and reuse.

All preset builders return:
...
The presets module follows the same overall design principles as the package:
...
These presets are not intended to be minimax-optimal constructions. They are practical, readable starting points for prototyping and intuition building before moving to more specialised polynomial design workflows.
...
This function is odd and bounded in magnitude by $1$, while behaving like a regularised inverse away from the origin. That makes it useful as a small, smooth inverse-like preset in QSVT experiments.
...
```python
import numpy as np
from qsvt.presets import inverse_like_polynomial
...
Because the target depends on $|x|$, the resulting polynomial is even. This makes it a natural singular-value filter preset when you want pass/reject behaviour without a discontinuous hard cutoff.

### Example

```python
import numpy as np
from qsvt.presets import soft_threshold_filter_polynomial
...
Approximating $\sqrt{x}$ directly on all of $[-1,1]$ is not appropriate, since $\sqrt{x}$ is only real for $x \ge 0$. The shifted form avoids that issue while still giving a useful square-root style preset for experiments.

### Example

```python
import numpy as np
from qsvt.presets import sqrt_approximation_polynomial
...
This function is strictly positive and bounded by $1$ on $[-1,1]$, making it a useful smooth weighting preset for spectral damping or amplification-style experiments.
...
```python
import numpy as np
from qsvt.presets import exponential_approximation_polynomial
...
The preset builders use a lightweight common fitting path:
...
Some presets have built-in symmetry:
...
This matters when using the presets for QSP/QSVT intuition, since many transformations naturally inherit even or odd structure.

## Typical workflow

A common pattern is:

1. choose a ready-made preset
...
The `qsvt.presets` and `qsvt.design` modules are closely related, but they serve slightly different purposes.

Use `qsvt.presets` when you want:
...
In practice, `qsvt.presets` is the simpler “grab a useful family and try it” layer, while `qsvt.design` is the more explicit “construct a polynomial for a particular task” layer.

## Notes and limitations

These presets are intentionally lightweight. They do not currently provide:
...
```python
from qsvt.presets import inverse_like_diagnostics
...
The `qsvt.presets` module provides compact bounded polynomial families for common QSVT/QSP-style tasks:
...
It is best viewed as a practical preset layer: easy to use, easy to inspect, and well suited to small experiments, notebook workflows, and intuition building before moving to more specialised polynomial design methods.
